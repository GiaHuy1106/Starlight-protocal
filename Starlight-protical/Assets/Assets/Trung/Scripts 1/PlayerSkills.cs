...
public class PlayerSkill : MonoBehaviour
{
    ...
    [Header("Meteor Range")]
    public float meteorRange = 8f; // khoảng cách tối đa cast meteor
    public float rangeCircleScaleFactor = 0.14f;// scale vòng tròn hiển thị phạm vi
    [Header("Skill Damage")]
    public int fireballSkillDamage = 50;
    ...
    {
        isAimingSkill = false;
        SyncSkillDamageFromPrefabs();
    }

    void SyncSkillDamageFromPrefabs()
    {
        // Đồng bộ damage skill từ prefab để tooltip và damage thực trùng nhau
        if (fireballPrefab != null)
        {
            var fb = fireballPrefab.GetComponent<FireballProjectile>() 
                     ?? fireballPrefab.GetComponentInChildren<FireballProjectile>();
            if (fb != null)
            {
                fireballSkillDamage = fb.damage;
            }
        }

        if (specialPrefab != null)
        {
            var meteor = specialPrefab.GetComponent<MeterorSkillDamage>() 
                         ?? specialPrefab.GetComponentInChildren<MeterorSkillDamage>();
            if (meteor != null)
            {
                specialSkillDamage = meteor.damage;
            }
        }
    }
    void Update()
        ...
        public void SpawnSpecial()
    {
        ...
        if (meteorDamage != null)
        {
            meteorDamage.damage = specialSkillDamage;
        }
    ...
    public void ShootFireball()
    {
        ...
        if (playerAttack.sfxSource != null && playerAttack.whooshSFX != null)
        {
            playerAttack.sfxSource.PlayOneShot(playerAttack.whooshSFX, playerAttack.whooshVolume);
        }
